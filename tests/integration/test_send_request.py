...
_transformers_mocks = {
    ...
    "transformers.dynamic_module_utils": MagicMock(),
...
def replace_imports(src):
    ...
    result = re.sub(
        r"from \.\w+ import \([^)]*\)",
        "# relative import replaced by test harness",
        src,
        flags=re.DOTALL,
    )
    # Single-line imports: from .xxx import yyy -> comment
    result = re.sub(
        r"from \.\w+ import .+", "# relative import replaced by test harness", result
    )
    return result
...
class TestSendRequestRouting:
    ...
    async def test_llamacpp_dispatches_correctly(self, mock_aioresponse):
        mock_aioresponse["json"].return_value = {
            "choices": [{"message": {"content": "llamacpp response"}}]
        }
...
class TestTransformersRouting:
    ...
    async def test_transformers_dispatches_correctly(self):
        ...
        _PROVIDER_REGISTRY["transformers"] = (
            mock_handler,
            _module_ns["_build_transformers_kwargs"],
        )
...
class TestImageConversionErrors:
    ...
    async def test_image_conversion_failure_returns_error(self):
        ...
        with patch.dict(
            _module_ns,
            {
                "convert_images_for_api": MagicMock(
...
class TestSendRequestCompleteFlow:
    ...
    async def test_complete_transformers_flow(self):
        ...
        _PROVIDER_REGISTRY = _module_ns["_PROVIDER_REGISTRY"]
        mock_handler = AsyncMock()
        mock_handler.return_value = {"choices": [{"message": {"content": "Success"}}]}
        _PROVIDER_REGISTRY["transformers"] = (
            mock_handler,
            _module_ns["_build_transformers_kwargs"],
        )
