...
class IFLLM:
    ...
    def __init__(self):
        ...
        self.neg_prompts = self.load_presets(
            os.path.join(self.presets_dir, "neg_prompts.json")
        )
        self.embellish_prompts = self.load_presets(
            os.path.join(self.presets_dir, "embellishments.json")
        )
        self.style_prompts = self.load_presets(
            os.path.join(self.presets_dir, "style_prompts.json")
        )
        self.stop_strings = self.load_presets(
            os.path.join(self.presets_dir, "stop_strings.json")
        )
            ...
            "optional": {
                ...
                "prime_directives": (
                    "STRING",
                    {
                        "default": "",
                        "forceInput": True,
                        "tooltip": "The system prompt for the LLM.",
                    },
                ),
                "profiles": (
                    ["None"] + list(node.profiles.keys()),
                    {
                        "default": "None",
                        "tooltip": "The pre-defined system_prompt from the json profile file on the presets folder you can edit or make your own will be listed here.",
                    },
                ),
                "embellish_prompt": (
                    list(node.embellish_prompts.keys()),
                    {
                        "default": "None",
                        "tooltip": "The pre-defined embellishment from the json embellishments file on the presets folder you can edit or make your own will be listed here.",
                    },
                ),
                "style_prompt": (
                    list(node.style_prompts.keys()),
                    {
                        "default": "None",
                        "tooltip": "The pre-defined style from the json style_prompts file on the presets folder you can edit or make your own will be listed here.",
                    },
                ),
                "neg_prompt": (
                    list(node.neg_prompts.keys()),
                    {
                        "default": "None",
                        "tooltip": "The pre-defined negative prompt from the json neg_prompts file on the presets folder you can edit or make your own will be listed here.",
                    },
                ),
                "stop_string": (
                    list(node.stop_strings.keys()),
                    {
                        "default": "None",
                        "tooltip": "Specifies a string at which text generation should stop.",
                    },
                ),
                "max_tokens": (
                    "INT",
                ...
                "random": (
                    "BOOLEAN",
                    {
                        "default": False,
                        "label_on": "Seed",
                        "label_off": "Temperature",
                        "tooltip": "Toggles between using a fixed seed or temperature-based randomness.",
                    },
                ),
                "seed": (
                    "INT",
                    {"default": 0, "tooltip": "Random seed for reproducible outputs."},
                ),
                "keep_alive": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "label_on": "Keeps Model on Memory",
                        "label_off": "Unloads Model from Memory",
                        "tooltip": "Determines whether to keep the model loaded in memory between calls.",
                    },
                ),
                "clear_history": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "label_on": "Clear History",
                        "label_off": "Keep History",
                        "tooltip": "Determines whether to clear the history between calls.",
                    },
                ),
                "history_steps": (
                    "INT",
                    {"default": 10, "tooltip": "Number of steps to keep in history."},
                ),
                "aspect_ratio": (
                    ["1:1", "16:9", "4:5", "3:4", "5:4", "9:16"],
                    {
                        "default": "1:1",
                ...
                "auto": (
                    ...
                    {
                        "default": False,
                        "label_on": "Auto Is Enabled",
                        "label_off": "Auto is Disabled",
                        "tooltip": "If true, it generates auto promts based on the listed images click the save Auto settings to set the auto prompt generation file",
                    },
                ),
                "batch_count": (
                    "INT",
                    {
                        "default": 1,
                        "tooltip": "Number of images to generate. only for create, edit and variations strategies.",
                    },
                ),
                "external_api_key": (
                    "STRING",
                    {
                        "default": "",
                        "tooltip": "If this is not empty, it will be used instead of the API key from the .env file. Make sure it is empty to use the .env file.",
                    },
                ),
                "Omni": (
                    "OMNI",
                    {
                        "default": None,
                        "tooltip": "Additional input for the selected tool.",
                    },
                ),
                "attention": (
                    ["sdpa", "flash_attention_2", "xformers"],
                    {
                        "default": "sdpa",
                        "tooltip": "Select attention mechanism on Transformer models.",
                    },
                ),
            },
            "hidden": {
                "temperature": (
                    "FLOAT",
                    {
                        ...
                        "min": 0.0,
                        "max": 1.0,
                        "tooltip": "Controls randomness in output generation. Higher values increase creativity but may reduce coherence.",
                    },
                ),
                "top_k": (
                    "INT",
                    {
                        "default": 40,
                        "tooltip": "Limits the next token selection to the K most likely tokens.",
                    },
                ),
                "top_p": (
                    "FLOAT",
                ...
                "repeat_penalty": (
                    ...
                    {
                        "default": 1.1,
                        "tooltip": "Penalizes repetition in generated text.",
                    },
                ),
                "precision": (
                    ["fp16", "fp32", "bf16"],
                    {
                        "default": "fp16",
                        "tooltip": "Select precision on Transformer models.",
                    },
                ),
            },
        }
        ...
        try:
            ...
            if external_api_key != "":
                ...
                llm_api_key = get_api_key(
                    f"{llm_provider.upper()}_API_KEY", llm_provider
                )
            ...
            embellish_content = (
                self.embellish_prompts.get(embellish_prompt, "").strip()
                if embellish_prompt
                else ""
            )
            style_content = (
                self.style_prompts.get(style_prompt, "").strip() if style_prompt else ""
            )
            neg_content = (
                self.neg_prompts.get(neg_prompt, "").strip() if neg_prompt else ""
            )
            ...
            if images is not None and len(images) > 0:
                ...
                logger.info(
                    "No images connected; continuing with text-based tasks only."
                )
        ...
        **kwargs,
        ...
        try:
            ...
            image_batches, mask_batches = process_auto_mode_images(
                images=images, mask=mask, batch_size=batch_size
            )
            ...
            for img_batch, mask_batch in zip(image_batches, mask_batches):
                for i in range(img_batch.size(0)):
                    ...
                    combo_prompt = await self.generate_combo_prompts(
                        images=single_img, settings_dict=None
                    )
                    ...
                    for iteration in range(batch_count):
                        batch_results = await self.process_auto_batch(
                            ...
                            **{
                                **kwargs,
                                "batch_count": 1,
                                "seed": kwargs.get("seed", 0) + iteration
    ...
    async def process_auto_batch(
        ...
        **kwargs,
        ...
        try:
            batch_kwargs = {k: v for k, v in kwargs.items() if k not in ["user_prompt"]}
    ...
    async def execute_normal_strategy(
        self,
        user_prompt,
        current_images,
        current_mask,
            ...
            for i in range(batch_count):
                try:
                    current_seed = (
                        ...
                        and kwargs.get("seed") is not None
                    ...
                    if kwargs.get("neg_prompt") == "AI_Fill":
                        neg_prompt = await self.generate_negative_prompt(
                            cleaned_response, images=current_images, **kwargs
                        )
                    else:
            ...
            for batch_idx in range(batch_count):
                try:
                    llm_response = await _get_send_request()(
                        ...
                        user_message=user_prompt,
                        messages=messages,
                        seed=kwargs.get("seed", 0) + batch_idx
                        if kwargs.get("seed", 0) != 0
                        else kwargs.get("seed", 0),
                        temperature=kwargs.get("temperature", 0.7),
                        max_tokens=kwargs.get("max_tokens", 2048),
                        random=kwargs.get("random", False),
                        top_k=kwargs.get("top_k", 40),
                    ...
                    if kwargs.get("neg_prompt") == "AI_Fill":
                        neg_prompt = await self.generate_negative_prompt(
                            llm_response, images=current_images, **kwargs
                        )
                    else:
                        neg_prompt = kwargs.get("neg_content", "")
                    ...
                    logger.error(
                        f"Error in OMOST batch {batch_idx}: {str(batch_error)}"
                    )
                    continue
    ...
    async def execute_create_strategy(self, user_prompt, current_mask, **kwargs):
        try:
            ...
            if all_base64_images:
                image_data = {"data": [{"b64_json": img} for img in all_base64_images]}
                ...
                image_tensor, mask_tensor = load_placeholder_image(
    ...
    async def execute_gemini2_create_strategy(
        ...
        try:
            if not GEMINI_SDK_AVAILABLE:
                ...
                return self.create_error_response(
                    current_images, current_mask, error_msg, user_prompt
                )
            ...
            logger.info(
                f"Using Gemini 2.0 Create strategy with seed: {seed}, temperature: {temperature}"
            )
                ...
                for idx, img in enumerate(input_images):
                    try:
                        ...
                        temp_img_path = os.path.join(
                            tempfile.gettempdir(),
                            f"gemini_input_{idx}_{int(time.time())}.png",
                        )
                        ...
                        logger.error(
                            f"Error processing input image {idx}: {str(img_error)}"
                        )
            ...
            logger.info(
                f"Calling Gemini API with {len(contents) if isinstance(contents, list) else 1} content parts"
            )
                ...
                for part in candidate.content.parts:
                    ...
                    if hasattr(part, "inline_data") and part.inline_data:
                        try:
                            ...
                            logger.info(
                                f"Extracted image {len(generated_images)} from response"
                            )
                        except Exception as img_error:
                            logger.error(
            ...
            for temp_path in temp_img_paths:
                try:
                    ...
                    logger.warning(
                        f"Failed to remove temporary file {temp_path}: {str(e)}"
                    )
            ...
            logger.info(
                f"Successfully processed {len(generated_images)} generated images"
            )
            ...
            logger.error(
                f"Error in Gemini 2.0 create strategy: {str(e)}", exc_info=True
            )
            return self.create_error_response(
                current_images,
                current_mask,
    ...
    def load_presets(self, file_path: str) -> Dict[str, Any]:
        ...
        for encoding in encodings:
            try:
                with codecs.open(file_path, "r", encoding=encoding) as f:
                    ...
                    if encoding.lower() not in ("utf-8", "utf-8-sig"):
                        try:
                            ...
                            logger.warning(
                                f"Could not write back UTF-8 encoded file: {write_err}"
                            )
                ...
                logger.error(
                    f"Error loading presets from {file_path} with {encoding} encoding: {e}"
                )
                continue
    ...
    async def generate_negative_prompt(
        self, prompt: str, images: List[Any], **kwargs
    ) -> List[str]:
        ...
        try:
            ...
            if isinstance(neg_prompt, dict):
                ...
                if "choices" in neg_prompt and neg_prompt["choices"]:
                    extracted = (
                        neg_prompt["choices"][0].get("message", {}).get("content", "")
                    )
    ...
    def process_image_wrapper(self, **kwargs):
        ...
        try:
                ...
                for result_item in result:
                    if isinstance(result_item, dict):
                        ...
                        if not isinstance(img, torch.Tensor):
                            placeholder_img, placeholder_mask = load_placeholder_image(
                                self.placeholder_image_path
                            )
                            img = placeholder_img
                        ...
                        if isinstance(img, torch.Tensor):
                            if img.dim() == 4:
                                if img.shape[1] == 4:
                                    img = img[:, :3, :, :]
                                    logger.debug(
                                        f"Converted batch of 4-channel images to 3-channel, new shape: {img.shape}"
                                    )
                            elif img.dim() == 3:
                                if img.shape[0] == 4:
                                    img = img[:3, :, :]
                                    logger.debug(
                                        f"Converted single 4-channel image to 3-channel, new shape: {img.shape}"
                                    )
                        ...
                        raise ValueError(
                            f"Unexpected result format: {type(result_item)}"
                        )
            ...
            retrieved_images_tensor = (
                torch.cat(retrieved_images, dim=0)
                if retrieved_images
            ...
            for idx in range(len(retrieved_images)):
                logger.debug(
                    f"Result {idx + 1}: Retrieved image type: {type(retrieved_images[idx])}"
                )
                if isinstance(retrieved_images[idx], torch.Tensor):
                    logger.debug(
                        f"Result {idx + 1}: Retrieved image shape: {retrieved_images[idx].shape}"
                    )
