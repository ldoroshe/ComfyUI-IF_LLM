...
async def send_gemini_request(
    ...
    gemini_messages = prepare_gemini_messages(
    ...
    if tools:
        data["generationConfig"]["tools"] = [
            {"functionDeclarations": tools}
        ]  # Changed to functionDeclarations
...
def prepare_gemini_messages(base64_images, system_message, user_message, messages):
    ...
    if base64_images:
        gemini_messages.append(
            build_multimodal_user_message(
                user_message, base64_images, image_format=ImageFormat.GEMINI
            )
        )
    else:
